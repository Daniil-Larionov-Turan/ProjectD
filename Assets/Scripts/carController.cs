...
public class carController : MonoBehaviour{
    ...
    void runCar(){
        ...
        rigidbody.angularDrag = (KPH > 1000)? KPH / 1000 : 0;
        rigidbody.drag = dragAmount + (KPH / 40000) ; //

        KPH = rigidbody.velocity.magnitude * 36f;
        friction();
    }
    ...
    void manual(){
        ...
        if((Input.GetAxis("Fire2") == 1  ) && gearNum <= gears.Length && Time.time >= gearChangeRate ){
            ...
            setEngineLerp(engineRPM - ( engineRPM > 15000 ? 20000 : 7000));
            //audio.DownShift();
        ...
        if((Input.GetAxis("Fire3") == 1 ) && gearNum >= 1  && Time.time >= gearChangeRate){
            gearChangeRate = Time.time + 1f/3f ;
            gearNum --;
            setEngineLerp(engineRPM - ( engineRPM > 15000 ? 15000 : 7000));
            //audio.DownShift();
        }
